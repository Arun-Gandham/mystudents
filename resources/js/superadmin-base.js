...
const savedBrand = localStorage.getItem('sa_brand') || getComputedStyle(root).getPropertyValue('--brand-primary').trim();
htmlEl.setAttribute('data-bs-theme', savedTheme);
root.style.setProperty('--brand-primary', savedBrand);
...
document.querySelectorAll('[data-brand]').forEach(btn => {
    btn.addEventListener('click', () => {
        const color = btn.getAttribute('data-brand');
        root.style.setProperty('--brand-primary', color);
        localStorage.setItem('sa_brand', color);
    });
});
...
function renderResults(items) {
    if (!results) return;
    if (!items.length) { results.classList.remove('show'); results.innerHTML = ''; return; }
...
input?.addEventListener('input', () => {
    ...
    const filtered = MOCK.filter(x => (x.name.toLowerCase().includes(q) || x.kind.toLowerCase().includes(q)));
