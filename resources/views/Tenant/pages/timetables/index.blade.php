                    ...
                    @foreach($timetables as $sectionId => $sectionTimetables)
                        <li class="nav-item mb-2" role="presentation">
                            <button class="nav-link w-100 {{ $loop->first ? 'active' : '' }}" 
                                ...
                                {{ $sectionTimetables->first()->section->grade->name ?? 'No Grade' }} - 
                                {{ $sectionTimetables->first()->section->name ?? 'No Section' }}
                            </button>
                        </li>
                    @endforeach
                </ul>
                    ...
                    @foreach($timetables as $sectionId => $sectionTimetables)
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" 
                            ...
                            @if($sectionTimetables->isEmpty())
                                <p class="text-muted">No timetables for this section yet.</p>
                            @else
